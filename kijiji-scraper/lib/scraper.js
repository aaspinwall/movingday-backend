...
function castVal(val) {
  ...
  if (val === "true") return true;
    ...
    return Number(val);
  else if (!isNaN(Date.parse(val))) return new Date(val);
  else return val;
}
  ...
  return new Promise(function (resolve, reject) {
    ...
    fetch(url)
      ...
      .then(function (body) {
        ...
        if (!adInfo)
          return reject(
            new Error(`Ad not found or invalid Kijiji HTML at ${url}`)
          );
        adInfo.url = url;
        resolve(adInfo);
      })
